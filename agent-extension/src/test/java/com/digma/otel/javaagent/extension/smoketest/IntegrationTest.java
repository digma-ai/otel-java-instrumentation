  ...
  static void setup() {
    backend =
            new GenericContainer<>(
                    ...
                    .withLogConsumer(new Slf4jLogConsumer(LoggerFactory.getLogger("IntegrationTest.backend")));
    backend.start();
  }

  protected GenericContainer<?> target;

  void startTarget(String extensionLocation, int jdk) {
    target = buildTargetContainer(agentPath, extensionLocation,jdk);
    target.start();
  }

  void startTargetWithExtendedAgent(int jdk) {
    target = buildTargetContainer(extendedAgentPath, null, jdk);
    target.start();
  }
  ...
  private GenericContainer<?> buildTargetContainer(String agentPath, String extensionLocation,
    ...
    GenericContainer<?> result =
            new GenericContainer<>(getTargetImage(jdk))
                    .withExposedPorts(8080)
                    .withNetwork(network)
                    .withLogConsumer(new Slf4jLogConsumer(LoggerFactory.getLogger("IntegrationTest.target")))
